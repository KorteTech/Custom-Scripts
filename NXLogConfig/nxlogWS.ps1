$vmlist = Get-Content C:\Github\KorteTech\Custom-Scripts\NXLogConfig\WSs.txt
foreach($VM in $VMlist) {
    Write-Output $VM

    switch ($VM)
    {
     {($_ -eq "TKCDATA") -or ($_ -eq "CITRIX2")} {
    ...
    default {
        Copy-Item -Path "\\kortedata\departments\Technologies\Installs\nxlog\Workstation Config\logs-01.loggly.com_sha12.crt" -Destination "\\$VM\c$\Program Files (x86)\nxlog\cert\logs-01.loggly.com_sha12.crt"
        Copy-Item -Path "\\kortedata\departments\Technologies\Installs\nxlog\Workstation Config\nxlog.conf" -Destination "\\$VM\c$\Program Files (x86)\nxlog\conf\nxlog.conf"
